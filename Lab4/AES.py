nr = 10

sbox = [
    
]

inv_sbox = [
    
]

rcon = [[0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
        ]


def encrypt(input_data, key):
    result = []
    data = list(map(lambda x: ord(x), input_data))
    if len(input_data) <= 16:
        data = add_spaces(data)
        encrypted = encrypt_block(data, key)
        result = list(map(lambda x: chr(x), encrypted))
        return ''.join(result)
    data = list(map(lambda x: ord(x), input_data))
    data_block = chunk_data(data, 16)
    encypted_blocks = []
    for b in data_block:
        if len(b) < 16:
            b = add_spaces(b)
        encypted_blocks.append(encrypt_block(b, key))
    for i in encypted_blocks:
        for j in i:
            result.append(chr(j))
    return ''.join(result)


def decrypt(input_data, key):
    result = []
    data = list(map(lambda x: ord(x), input_data))
    if len(input_data) <= 16:
        data = add_spaces(data)
        encrypted = decrypt_block(data, key)
        result = list(map(lambda x: chr(x), encrypted))
        return ''.join(result)
    data = list(map(lambda x: ord(x), input_data))
    data_block = chunk_data(data, 16)
    decrypted_block = []
    for b in data_block:
        if len(b) < 16:
            b = add_spaces(b)
        decrypted_block.append(decrypt_block(b, key))

    for i in decrypted_block:
        for j in i:
            result.append(chr(j))
    return ''.join(result)


def chunk_data(data, length):
    return list((data[0 + i:length + i] for i in range(0, len(data), length)))


def encrypt_block(input_data, key):
    state = [[] for _ in range(4)]
    for r in range(4):
        for c in range(4):
            state[r].append(input_data[r + 4 * c])
    key_schedule = key_expansion(key)
    state = add_round_key(state, key_schedule, 0)
    for round in range(1, nr):
        state = sub_bytes(state, False)
        state = shift_rows(state, False)
        state = mix_columns(state, False)
        state = add_round_key(state, key_schedule, round)
    state = sub_bytes(state, False)
    state = shift_rows(state, False)
    state = add_round_key(state, key_schedule, round + 1)
    result = [None for _ in range(4 * 4)]
    for r in range(4):
        for c in range(4):
            result[r + 4 * c] = state[r][c]

    return result


def decrypt_block(input_data, key):
    state = [[] for _ in range(4)]
    for r in range(4):
        for c in range(4):
            state[r].append(input_data[r + 4 * c])
    key_schedule = key_expansion(key)
    state = add_round_key(state, key_schedule, nr)
    round = nr - 1
    while round >= 1:
        state = shift_rows(state, True)
        state = sub_bytes(state, True)
        state = add_round_key(state, key_schedule, round)
        state = mix_columns(state, True)
        round -= 1

    state = shift_rows(state, True)
    state = sub_bytes(state, True)
    state = add_round_key(state, key_schedule, round)

    result = [None for _ in range(4 * 4)]
    for r in range(4):
        for c in range(4):
            result[r + 4 * c] = state[r][c]
    return result


def sub_bytes(state, inv):
    if not inv:
        data_arr = sbox
    else:
        data_arr = inv_sbox
    for i in range(len(state)):
        for j in range(len(state[i])):
            r = state[i][j] // 16
            c = state[i][j] % 16
            elem = data_arr[16 * r + c]
            state[i][j] = elem

    return state


def shift_rows(state, inv):
    count = 1
    if not inv:
        for i in range(1, 4):
            state[i] = left_shift(state[i], count)
            count += 1
    else:
        for i in range(1, 4):
            state[i] = right_shift(state[i], count)
            count += 1

    return state


def mix_columns(state, inv):
    for i in range(4):
        if not inv:
            s0 = mul_by_02(state[0][i]) ^ mul_by_03(state[1][i]) ^ state[2][i] ^ state[3][i]
            s1 = state[0][i] ^ mul_by_02(state[1][i]) ^ mul_by_03(state[2][i]) ^ state[3][i]
            s2 = state[0][i] ^ state[1][i] ^ mul_by_02(state[2][i]) ^ mul_by_03(state[3][i])
            s3 = mul_by_03(state[0][i]) ^ state[1][i] ^ state[2][i] ^ mul_by_02(state[3][i])
        else:
            s0 = mul_by_0e(state[0][i]) ^ mul_by_0b(state[1][i]) ^ mul_by_0d(state[2][i]) ^ mul_by_09(state[3][i])
            s1 = mul_by_09(state[0][i]) ^ mul_by_0e(state[1][i]) ^ mul_by_0b(state[2][i]) ^ mul_by_0d(state[3][i])
            s2 = mul_by_0d(state[0][i]) ^ mul_by_09(state[1][i]) ^ mul_by_0e(state[2][i]) ^ mul_by_0b(state[3][i])
            s3 = mul_by_0b(state[0][i]) ^ mul_by_0d(state[1][i]) ^ mul_by_09(state[2][i]) ^ mul_by_0e(state[3][i])

        state[0][i] = s0
        state[1][i] = s1
        state[2][i] = s2
        state[3][i] = s3

    return state


def add_round_key(state, key_schedule, round):
    for col in range(4):
        s0 = state[0][col] ^ key_schedule[0][4 * round + col]
        s1 = state[1][col] ^ key_schedule[1][4 * round + col]
        s2 = state[2][col] ^ key_schedule[2][4 * round + col]
        s3 = state[3][col] ^ key_schedule[3][4 * round + col]

        state[0][col] = s0
        state[1][col] = s1
        state[2][col] = s2
        state[3][col] = s3

    return state


def key_expansion(key):
    key_symbols = [ord(symbol) for symbol in key]

    if len(key_symbols) < 4 * 4:
        for i in range(4 * 4 - len(key_symbols)):
            key_symbols.append(0x01)

    key_schedule = [[] for _ in range(4)]
    for r in range(4):
        for c in range(4):
            key_schedule[r].append(key_symbols[r + 4 * c])

    for col in range(4, 4 * (nr + 1)):
        if col % 4 == 0:
            tmp = [key_schedule[row][col - 1] for row in range(1, 4)]
            tmp.append(key_schedule[0][col - 1])

            for j in range(len(tmp)):
                sbox_row = tmp[j] // 0x10
                sbox_col = tmp[j] % 0x10
                sbox_elem = sbox[16 * sbox_row + sbox_col]
                tmp[j] = sbox_elem

            for row in range(4):
                s = (key_schedule[row][col - 4]) ^ (tmp[row]) ^ (rcon[row][int(col / 4 - 1)])
                key_schedule[row].append(s)
        else:
            for row in range(4):
                s = key_schedule[row][col - 4] ^ key_schedule[row][col - 1]
                key_schedule[row].append(s)

    return key_schedule


def left_shift(array, count):
    res = array[:]
    for i in range(count):
        temp = res[1:]
        temp.append(res[0])
        res[:] = temp[:]
    return res


def right_shift(array, count):
    res = array[:]
    for i in range(count):
        tmp = res[:-1]
        tmp.insert(0, res[-1])
        res[:] = tmp[:]
    return res


def mul_by_02(num):
    if num < 0x80:
        res = (num << 1)
    else:
        res = (num << 1) ^ 0x1b
    return res % 0x100


def mul_by_03(num):
    return mul_by_02(num) ^ num


def mul_by_09(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ num


def mul_by_0b(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(num) ^ num


def mul_by_0d(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(mul_by_02(num)) ^ num


def mul_by_0e(num):
    return mul_by_02(mul_by_02(mul_by_02(num))) ^ mul_by_02(mul_by_02(num)) ^ mul_by_02(num)


def add_spaces(data):
    for i in range(4 * 4 - len(data)):
        data.append(32)
    return data
